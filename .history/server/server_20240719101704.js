...
app.post('/register', upload.single('resume') [
  ...
  try {
    ...
    const checkQuery = 'INSERT INTO users (username, password, email, resume_path) VALUES (?, ?, ?, ?)';
    
    db.query(checkQuery, [username, email, hashedPassword, resume_path], (checkError, checkResults) => {
      if (checkError) {
