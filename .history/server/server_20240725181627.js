...
initializePassport(passport); // Initialize passport with the JWT strategy
...
app.get('/user/:id/recommended-articles', passport.authenticate('jwt', { session: false }), async (req, res) => {
  console.log(error);
  ...
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      model: 'gpt-4o-mini',
      messages: [{ role: "user", content: `Based on this resume: "${resumeText}" please recommend 5 current articles that would be of interest to this person. Provide the response in JSON format with "title", "description", and "url".` }],
      max_tokens: 3000,
      functions: [{
        "name": "get_recommended_articles",
        "description": "Get recommended articles based on the resume text",
        "parameters": {
          "type": "object",
          "properties": {
            "title": {
      ...
      function_call: "get_recommended_articles"
    }, {
      headers: {
    ...
    res.status(500).json({ message: 'Error getting article recommendations', error: error.message });
  }
});
