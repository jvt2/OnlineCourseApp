  ...
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      messages: [{"role": "system", "content": "You are a helpful assistant designed to ourput JSON."},
        { role: "user", content: `Based on this resume: "${resumeText}" please recommend 5 current articles that would be of interest to this person.` }],
      max_tokens: 3000,
    }, {
      headers: {
    ...
    const messageContent = response.data.choices[0].message.content;
    const jsonString = messageContent.match(/```json([\s\S]*)```/)[1].trim();
    const articles = JSON.parse(jsonString).articles;
